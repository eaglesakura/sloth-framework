...
/**
 * public以外と継承に対応した拡張Finder
 * Package Privateなクラスを利用するため、package名はottoに合わせる
 */
public class AnnotatedHandlerFinder2 {
    ...
    private static void loadAnnotatedMethods(Class<?> listenerClass,
        ...
        List<Method> subscribeMethods = ReflectionUtil.listAnnotationMethods(listenerClass, Subscribe.class);
        List<Method> produceMethods = ReflectionUtil.listAnnotationMethods(listenerClass, Produce.class);

        List<Method> allMethods = new ArrayList<>();
        allMethods.addAll(subscribeMethods);
        allMethods.addAll(produceMethods);
...
//                }
    ...
    private AnnotatedHandlerFinder2() {
        // No instances.
    }

    public static HandlerFinder newInstance() {
        return new HandlerFinder() {
            @Override
            public Map<Class<?>, EventProducer> findAllProducers(Object listener) {
                return AnnotatedHandlerFinder2.findAllProducers(listener);
            }

            @Override
            public Map<Class<?>, Set<EventHandler>> findAllSubscribers(Object listener) {
                return AnnotatedHandlerFinder2.findAllSubscribers(listener);
            }
        };
    }
}
